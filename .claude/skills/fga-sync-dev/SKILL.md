...
- Never log raw payloads, bearer tokens, or anything that may contain PII, at any level. Do not log `string(message.Data())` or the raw `response`: the access-check and sync update-access payloads are tuple-shaped but embed user principals (often email-like identifiers, e.g. `object#relation@user:alice@example.com`), which is PII. The current `accessCheckHandler`, `genericUpdateAccessHandler`, and `processStandardAccessUpdate` call sites that log raw data at info are a known gap, not a pattern to copy. Log only non-PII structured fields (`subject`, `object`, `object_type`, `operation`, `relation`/`relations`, `count`); if a principal must appear for debugging, redact or hash it and keep it behind `DebugContext`.
